...
if temptable.WebSocket then
 ...
 if (game.Players.LocalPlayer.UserId == 4118463253) then
local req = syn.request({
    Url = "https://BSSConfigAPI.aggeloslua.repl.co/getconfig",
    Method = "POST",
    Headers = {
...
if (req.Success) then
    local s, e = pcall (function()
        local DecB = game:service'HttpService':JSONDecode(req.Body)
        kometa = DecB.Data.Config
    end)
    if (e) then
        warn("[ERROR]: Failed to load your configs!")
    end
...
if (req.Success) then
    ...
    if (e) then
        warn("[ERROR]: Failed to load your configs!"..e)
    end
